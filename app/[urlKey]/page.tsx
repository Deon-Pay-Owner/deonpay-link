...
export default async function PaymentLinkPage({
  ...
  console.log('[CheckoutCreate DEBUG] ====== FRONTEND CREATING CHECKOUT SESSION ======')
  console.log('[CheckoutCreate DEBUG] payment_link_id:', paymentLink.id)
  console.log('[CheckoutCreate DEBUG] merchant_id:', paymentLink.merchant_id)
  console.log('[CheckoutCreate DEBUG] total_amount:', totalAmount)
  console.log('[CheckoutCreate DEBUG] Note: url_key will be auto-generated by database')
  ...
  if (sessionError || !checkoutSession) {
    console.error('[CheckoutCreate DEBUG] ❌ Failed to create session:', {
  ...
  console.log('[CheckoutCreate DEBUG] ✅ Session created successfully:', {
    session_id: checkoutSession.id,
    session_url_key: checkoutSession.url_key,
  ...
  // Insert line items
  if (sessionLineItems.length > 0) {
    console.log('[CheckoutCreate DEBUG] Inserting', sessionLineItems.length, 'line items')
    ...
    if (lineItemsError) {
      console.error('[CheckoutCreate DEBUG] ❌ Error creating line items:', lineItemsError)
    } else {
      console.log('[CheckoutCreate DEBUG] ✅ Line items inserted successfully')
    }
  }
  ...
  if (!checkoutSession.url_key) {
    console.error('[CheckoutCreate DEBUG] ❌ No url_key in checkout session:', checkoutSession)
    throw new Error('Checkout session missing url_key')
  }

  console.log('[CheckoutCreate DEBUG] ====== REDIRECTING TO CHECKOUT ======')
  console.log('[CheckoutCreate DEBUG] Redirect URL: /checkout/' + checkoutSession.url_key)
  console.log('[CheckoutCreate DEBUG] Final summary:', {
    payment_link_id: paymentLink.id,
    session_id: checkoutSession.id,
    session_url_key: checkoutSession.url_key,
    redirect_path: `/checkout/${checkoutSession.url_key}`
  })
